...
test('creates and retrieves an interview room', async () => {
  ...
  const created = await request(app).post('/rooms').expect(201);
  ...
  const loaded = await request(app).get('/rooms/' + created.body.id).expect(200);
...
test('returns contract errors for missing rooms and invalid updates', async () => {
  ...
  const missing = await request(app).get('/rooms/missing').expect(404);
